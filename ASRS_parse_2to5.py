...
import pdfplumber
with pdfplumber.open("C:/Users/ljkop/Laura/ASRS/filename.pdf") as pdf:
...
def clean_table(df_temp, col_name):
    ...
    ## remove question number at beginning of sentence fragment
    ...
    df_temp['Item Score'] = df_temp['Item Score'].round(0)
...
tot_tscore = df_ASRS.iloc[10, 1]
tot_percent = df_ASRS.iloc[10, 2]
tot_class = df_ASRS.iloc[10, 3]

##  delete total row (last row)
df_ASRS = df_ASRS.drop(index=10).reset_index(drop=True)
...
df_ASRS['Scale'] = df_ASRS['Scale'].str.replace('Attention/ Self-Regulation', 'Attention/Self-Regulation')
...
##  delete rows 1-4; they are not used in the body table
df_ASRS = df_ASRS.drop(columns=df_ASRS.columns[[1,2,3,4]]).reset_index(drop=True)

## function to create a comment from the item table and add that comment to teh ASRS body table
## Choose the comment from the item table that is the first comment with a score of 3 or greater
...
## create a comment for each item
...
add_comment(df_asr, 'Attention/Self-Regulation')
